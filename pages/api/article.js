...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ② Webサーチ付きでClaude呼び出し
    const prompt = `以下の商品リストについて、実際のユーザーレビューや話題になっている理由をWeb検索で調べてから、在宅ワーカー向けのガジェットレビュー記事をJSON形式で作成してください。
...
ルール：
・Web検索で得た実際の情報・レビュー・話題を盛り込む
・友達に話すような自然な口調
・体験談風（「実際に使うと」「やっぱ」など）
・押しつけがましくない
・最後にJSONのみ出力・前後に余分な文字や説明不要・マークダウン記法不要
