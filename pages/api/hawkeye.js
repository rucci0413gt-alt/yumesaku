...
export default async function handler(req, res) {
  ...
  try {
    ...
    const list = products.map((p, i) =>
      `${i}. ${p.name} / ¥${p.price} / ★${p.review}(${p.reviewCount}件) / カテゴリ:${p.keyword}`
    ).join('\n');
...
{
  "picks": [
    {"index": 商品番号, "reason": "なぜエンジニアにおすすめか40文字", "xHook": "Xでバズりそうな一言フック30文字", "tag": "鉄板"}
  ],
