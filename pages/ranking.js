...
fetchRanking(categoryRef.current);
setNextUpdate(30 * 60);
}, 30 * 60 * 1000);

// カウントダウン（1秒ごと）
countdownRef.current = setInterval(() => {
setNextUpdate(prev => prev > 0 ? prev - 1 : 30 * 60);
}, 1000);
...
次の更新まで {formatCountdown(nextUpdate)}
</span>
</div>
<button
onClick={() => fetchRanking(category)}
disabled={loading}
...
.ranking-card:hover .product-image { transform: scale(1.08); }
.ranking-card:hover .arrow { transform: translateX(4px); opacity: 1; }
...
.review-count { color: #CCC; font-size: 0.9em; }
...
.nav { gap: 16px; }
.arrow { display: none; }
.ranking-card { padding: 12px; gap: 10px; border-radius: 12px; }
.status-info { gap: 8px; }
}
`}</style>
</>
